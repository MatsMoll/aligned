...
from aligned.retrieval_job import DerivedFeatureJob, RetrievalJob, RetrievalRequest
...
@pytest.fixture
def retrieval_request_without_derived() -> RetrievalRequest:
...
@pytest.fixture
def retrieval_job(retrieval_request_without_derived: RetrievalRequest) -> RetrievalJob:
    return FileFullJob(
        LiteralReference(
            pl.DataFrame(
                {"id": [1, 2, 3, 4, 5], "a": [3, 4, 2, 3, 4], "b": [1, 1, 1, 2, 4]}
            )
        ),
        request=retrieval_request_without_derived,
    )
...
@pytest.fixture
def retrieval_job_with_timestamp(
    retrieval_request_with_derived: RetrievalRequest,
) -> RetrievalJob:
...
@pytest.fixture
def breast_scan_feature_viewout_with_datetime(
    ...
    class BreastDiagnoseFeatureView:
        ...
        diagnosis = String().description(
            "The given diagnose. M for malignant, and B for benigne"
        )
        is_malignant = (diagnosis == "M").description(
            "If the scanned cells was diagnosed as dangerous"
        )
...
@pytest.fixture
def titanic_feature_view_parquet(
    titanic_source_parquet: ParquetFileSource,
) -> FeatureViewWrapper:
    @feature_view(
...
@pytest.fixture
def alot_of_transforations_feature_view(
    titanic_source: CsvFileSource,
) -> FeatureViewWrapper:
    @feature_view(
        name="titanic",
        description="Some features from the titanic dataset",
...
def titanic_feature_view_scd(titanic_source_scd: CsvFileSource) -> FeatureViewWrapper:
    ...
    @feature_view(
        ...
        stream_source=redis.stream(topic="titanic_stream").with_coder(
            JsonRecordCoder("json")
        ),
    )
    class TitanicPassenger:
        passenger_id = Int32().as_entity()
        ...
        name_embedding = name.embedding(
            EmbeddingModel.gensim("glove-wiki-gigaword-50")
        ).indexed(
            embedding_size=50,
            storage=redis.index(name="name_embedding_index"),
            metadata=[age, sex],
        )
...
def point_in_time_data_test() -> DataTest:
    ...
    second_event_timestamp = datetime(
        2020, 4, 27, 18, 1, 4, 746575, tzinfo=timezone.utc
    )
