...
class SupportedValueFactory:
    ...
    def __init__(self) -> None:
        types = [EnvironmentValue, LiteralValue, PlaceholderValue, NothingValue, ConcatValue]
...
class PlaceholderValue(ConfigValue, Codable):
    ...
    def read(self) -> str:
        raise NotImplementedError(f"Did not replace placeholder of '{self.placeholder_name}'")
...
class NothingValue(ConfigValue, Codable):
    ...
    def read(self) -> str:
        raise ValueError('Trying to access a config value that is expected to not exist.')
...
class ConcatValue(ConfigValue, Codable):
    ...
    def read(self) -> str:
        return self.seperator.join([val.read() for val in self.values])
...
class PathResolver(Codable):
    ...
    @staticmethod
    def from_value(values: list[ConfigValue] | str | PathResolver | ConfigValue) -> PathResolver:
