...
@model_contract(
    name="first_with_versions", input_features=[view.feature_a, other.feature_b]
)
...
class AnnotatedData:
    ...
    text = String()
    subject = String().as_annotated_feature()
...
def test_model_request() -> None:
    ...
    assert model_request.features_to_include == {
        "feature_a",
        "feature_b",
        "view_id",
        "other_id",
...
async def test_load_preds_with_different_model_version() -> None:
    ...
    df = await model_store.predictions_for(
        {"some_id": [1, 2], "model_version": ["v2", "v2"]}
    ).to_polars()
