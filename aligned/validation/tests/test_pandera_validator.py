...
@pytest.mark.asyncio
async def test_validate_valid_feature_view_polars(titanic_feature_store: ContractStore) -> None:
    original = await titanic_feature_store.feature_view('titanic').all(limit=5).to_pandas()
    validated_df = await titanic_feature_store.feature_view('titanic').all(limit=5).drop_invalid().to_polars()

    assert original.shape == validated_df.shape


@pytest.mark.asyncio
async def test_validate_invalid_feature_view_polars(titanic_feature_store: ContractStore) -> None:
    validated_df = (
        await titanic_feature_store.feature_view('titanic').all(limit=20).drop_invalid().to_polars()
    )

    assert validated_df.height == 16
