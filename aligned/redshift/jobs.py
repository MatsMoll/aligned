...
@dataclass
class FactRedshiftJob(RetrievalJob):
    ...
    async def psql_job(self) -> PostgreSqlJob:
        if isinstance(self.facts, PostgreSqlJob):
            return PostgreSqlJob(
                self.config.psql_config, self.build_sql_entity_query(self.facts)
            )
    ...
    def value_selection(
        ...
        if request.event_timestamp and entities_has_event_timestamp:
            ...
            event_timestamp_clause = f"{self.entity_table_name}.event_timestamp >= ta.{event_timestamp_column}"
        ...
        if source.list_references:
            for feature_name, reference in source.list_references.items():
                ...
                source_id_column = (
                    reference.join_column or list(request.entity_names)[0]
                )
                ...
                table_join_condition = (
                    f"{table_id}.{reference.id_column} = ta.{source_id_column}"
                )
    ...
    def sql_aggregated_request(
        ...
        if request.event_timestamp:
            ...
            event_timestamp_name = source.feature_identifier_for(
                [request.event_timestamp.name]
            )[0]
            if window.window:
                time_window_config = window.window
                ...
                event_timestamp_clause = f"ta.{event_timestamp_name} <= {self.entity_table_name}.event_timestamp"
        ...
        def resolve_sql_transformation(feature: str) -> str:
            ...
            for depended in hidden_feature.depending_on_names:
                ...
                if depended.isnumeric():
                    assert isinstance(
                        sub_feature.transformation, RedshiftTransformation
                    )
    ...
    def build_sql_entity_query(self, sql_facts: PostgreSqlJob) -> str:
        ...
        for request in self.requests:
            final_select_names = final_select_names.union(
                {
                    f"{self.entity_table_name}.{entity}"
